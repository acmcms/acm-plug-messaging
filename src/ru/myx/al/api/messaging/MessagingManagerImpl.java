...
final class MessagingManagerImpl implements MessagingManager {
	...
	final void commitMessageBlank(
			final String factoryId,
			final BaseObject factoryParameters,
			...
			try {
				...
				final Timestamp messageDate = new Timestamp( Engine.fastTime() );
				{
					try (final PreparedStatement ps = conn
							.prepareStatement( "INSERT INTO m1Messages(msgId,msgOwnerId,msgDate,fcId,fcDataType,fcData) VALUES (?,?,?,?,?,?)" )) {
				...
				{
					try (final PreparedStatement ps = conn
							.prepareStatement( "INSERT INTO m1Sent(msgId,msgUserId,msgTarget,msgProcessed) VALUES (?,?,?,?)" )) {
				...
				}
				{
					try (final PreparedStatement ps = conn
						...
						final String messageInteractive = interactive
								? "Y"
								: "N";
						final Timestamp messageExpire = new Timestamp( Engine.fastTime() + 1000L * 60L * 60L * 24L * 7L );
						for (final String recipient : recipients) {
							ps.setString( 1, messageId );
	...
	@Override
	public final MessageBlank createBlankMessage(
			final String messageFactoryId,
			final BaseObject messageFactoryParameters) {
	...
	public final void deleteInbox(final int[] keys) {
		try (final Connection conn = this.parent.nextConnection()) {
			...
			try (final PreparedStatement ps = conn.prepareStatement( "DELETE FROM m1Inbox WHERE msgLuid IN ("
					+ MessagingManagerImpl.join( keys )
					+ ")" )) {
				ps.execute();
			}
		} catch (final SQLException e) {
	...
	public final void deleteSent(final int[] keys) {
		try (final Connection conn = this.parent.nextConnection()) {
			try (final PreparedStatement ps = conn.prepareStatement( "DELETE FROM m1Sent WHERE msgLuid IN ("
					+ MessagingManagerImpl.join( keys )
					+ ")" )) {
				ps.execute();
			}
	...
	public final Message[] getInbox() {
		try (final Connection conn = this.parent.nextConnection()) {
			...
			final PreparedStatement ps = conn
					.prepareStatement( "SELECT i.msgLuid, i.msgId, m.msgDate, i.msgRead, i.msgTarget, m.msgOwnerId, m.fcId FROM m1Inbox i, m1Messages m WHERE i.msgId=m.msgId AND i.msgUserId=? ORDER BY m.msgDate DESC",
			...
			try {
				...
				try (final ResultSet rs = ps.executeQuery()) {
					if (rs.next()) {
						...
						do {
							result.add( new MessageImpl( this, rs.getInt( 1 ), rs.getString( 2 ), rs.getTimestamp( 3 )
									.getTime(), true, "Y".equals( rs.getString( 4 ) ), rs.getString( 5 ), rs
	...
	public final Message getInbox(final int key) {
		try (final Connection conn = this.parent.nextConnection()) {
			...
			try (final PreparedStatement ps = conn
					.prepareStatement( "SELECT i.msgLuid,i.msgId,m.msgDate,i.msgRead,i.msgTarget,m.msgOwnerId,m.fcId FROM m1Inbox i, m1Messages m WHERE i.msgId=m.msgId AND i.msgLuid=? AND i.msgUserId=?",
							ResultSet.TYPE_FORWARD_ONLY,
							ResultSet.CONCUR_READ_ONLY )) {
				ps.setInt( 1, key );
	...
	final BaseObject getMessageParameters(final String msgId) {
		try (final Connection conn = this.parent.nextConnection()) {
			...
			try (final PreparedStatement ps = conn
					.prepareStatement( "SELECT fcDataType,fcData FROM m1Messages WHERE msgId=?",
							ResultSet.TYPE_FORWARD_ONLY,
							ResultSet.CONCUR_READ_ONLY )) {
				ps.setString( 1, msgId );
				try (final ResultSet rs = ps.executeQuery()) {
	...
	public final Message[] getSent() {
		try (final Connection conn = this.parent.nextConnection()) {
			...
			try (final PreparedStatement ps = conn
					.prepareStatement( "SELECT s.msgLuid,s.msgId,m.msgDate,s.msgTarget,m.msgOwnerId,m.fcId FROM m1Sent s, m1Messages m WHERE s.msgId=m.msgId AND s.msgUserId=? ORDER BY m.msgDate DESC",
							ResultSet.TYPE_FORWARD_ONLY,
							ResultSet.CONCUR_READ_ONLY )) {
				ps.setString( 1, Context.getUserId( Exec.currentProcess() ) );
				try (final ResultSet rs = ps.executeQuery()) {
					if (rs.next()) {
						...
						do {
							result.add( new MessageImpl( this, rs.getInt( 1 ), rs.getString( 2 ), rs.getTimestamp( 3 )
									.getTime(), true, true, rs.getString( 4 ), rs.getString( 5 ), rs.getString( 6 ) ) );
						} while (rs.next());
						return result.toArray( new Message[result.size()] );
					}
					return null;
	...
	public final Message getSent(final int key) {
		try (final Connection conn = this.parent.nextConnection()) {
			final PreparedStatement ps = conn
					.prepareStatement( "SELECT s.msgLuid,s.msgId,m.msgDate,s.msgTarget,m.msgOwnerId,m.fcId FROM m1Sent s, m1Messages m WHERE s.msgId=m.msgId AND s.msgLuid=? AND s.msgUserId=? AND s.msgProcessed=?",
							ResultSet.TYPE_FORWARD_ONLY,
							ResultSet.CONCUR_READ_ONLY );
			try {
				ps.setInt( 1, key );
	...
	final void inboxRead(final int msgLuid, final String msgId) {
		try (final Connection conn = this.parent.nextConnection()) {
			try (final PreparedStatement ps = conn
					.prepareStatement( "UPDATE m1Inbox SET msgRead=? WHERE msgLuid=? AND msgId=?" )) {
				ps.setString( 1, "Y" );
				ps.setInt( 2, msgLuid );
